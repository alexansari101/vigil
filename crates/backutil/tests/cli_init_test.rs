...
"#,
    ...
    // Given the project structure, let's run `cargo run --bin backutil` with modified env vars.
    ...
    assert!(status.success(), "backutil init failed");
    ...
    assert!(status.success(), "backutil init idempotency check failed");
