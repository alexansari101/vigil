use anyhow::Result;
use serial_test::serial;
use std::fs;
use std::io::Write;
use std::process::{Command, Stdio};
use tempfile::TempDir;

#[test]
#[serial]
fn test_cli_setup_idempotent() -> Result<()> {
    let temp_dir = TempDir::new()?;
    let config_dir = temp_dir.path().join("config");
    let data_dir = temp_dir.path().join("data");

    let vigil_config_dir = config_dir.join("vigil");
    fs::create_dir_all(&vigil_config_dir)?;
    fs::create_dir_all(&data_dir)?;

    let config_file = vigil_config_dir.join("config.toml");
    let pass_path = vigil_config_dir.join(".repo_password");
    fs::write(&pass_path, "existingpass")?;

    let source_path = temp_dir.path().join("source");
    let target_path = temp_dir.path().join("target");
    let config_content = format!(
        r#"
[global]
debounce_seconds = 60

[[backup_set]]
name = "existing"
source = "{}"
target = "{}"
"#,
        source_path.display(),
        target_path.display()
    );
    fs::write(&config_file, config_content)?;

    let vigil_bin = env!("CARGO_BIN_EXE_vigil");

    let output = Command::new(vigil_bin)
        .arg("setup")
        .env("XDG_CONFIG_HOME", &config_dir)
        .env("XDG_DATA_HOME", &data_dir)
        .env("VIGIL_CONFIG", &config_file)
        .output()?;

    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("Password file found"));
    assert!(stdout.contains("Configuration found"));
    assert!(stdout.contains("existing"));

    Ok(())
}

#[test]
#[serial]
fn test_cli_setup_fresh() -> Result<()> {
    let temp_dir = TempDir::new()?;
    let config_dir = temp_dir.path().join("config");
    let data_dir = temp_dir.path().join("data");

    fs::create_dir_all(&config_dir)?;
    fs::create_dir_all(&data_dir)?;

    let vigil_bin = env!("CARGO_BIN_EXE_vigil");

    let mut child = Command::new(vigil_bin)
        .arg("setup")
        .env("XDG_CONFIG_HOME", &config_dir)
        .env("XDG_DATA_HOME", &data_dir)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()?;

    let stdin = child.stdin.as_mut().unwrap();

    // Step 1: Password
    writeln!(stdin, "testpass")?;
    writeln!(stdin, "testpass")?;

    // Step 2: Config
    let src_path = temp_dir.path().join("src");
    let tgt_path = temp_dir.path().join("tgt");
    writeln!(stdin, "testset")?;
    writeln!(stdin, "{}", src_path.display())?;
    writeln!(stdin, "{}", tgt_path.display())?;

    // Init offer?
    writeln!(stdin, "n")?;

    // Step 3: Service?
    writeln!(stdin, "n")?;

    let output = child.wait_with_output()?;
    assert!(output.status.success());

    let config_file = config_dir.join("vigil/config.toml");
    let password_file = config_dir.join("vigil/.repo_password");

    assert!(config_file.exists());
    assert!(password_file.exists());

    let config_content = fs::read_to_string(config_file)?;
    assert!(config_content.contains("testset"));

    Ok(())
}

#[test]
#[serial]
fn test_cli_setup_partial() -> Result<()> {
    let temp_dir = TempDir::new()?;
    let config_dir = temp_dir.path().join("config");
    let data_dir = temp_dir.path().join("data");

    let vigil_config_dir = config_dir.join("vigil");
    fs::create_dir_all(&vigil_config_dir)?;
    fs::create_dir_all(&data_dir)?;

    let config_file = vigil_config_dir.join("config.toml");
    let pass_path = vigil_config_dir.join(".repo_password");
    fs::write(&pass_path, "existingpass")?;

    let vigil_bin = env!("CARGO_BIN_EXE_vigil");

    let mut child = Command::new(vigil_bin)
        .arg("setup")
        .env("XDG_CONFIG_HOME", &config_dir)
        .env("XDG_DATA_HOME", &data_dir)
        .env("VIGIL_CONFIG", &config_file)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()?;

    let stdin = child.stdin.as_mut().unwrap();

    // Should skip password and go straight to config
    let src_path = temp_dir.path().join("src");
    let tgt_path = temp_dir.path().join("tgt");
    writeln!(stdin, "partialset")?;
    writeln!(stdin, "{}", src_path.display())?;
    writeln!(stdin, "{}", tgt_path.display())?;

    // Init offer?
    writeln!(stdin, "n")?;

    // Step 3: Service?
    writeln!(stdin, "n")?;

    let output = child.wait_with_output()?;
    assert!(output.status.success());

    assert!(config_file.exists());
    let config_content = fs::read_to_string(config_file)?;
    assert!(config_content.contains("partialset"));

    Ok(())
}
