...
async fn test_bootstrap_unit_file_generation() {
    ...
    // Use vigil_lib::paths to get the expected path
    let unit_path = vigil_lib::paths::systemd_unit_path();
    ...
    if unit_path.exists() {
        ...
        assert!(content.contains("Description=Vigil Daemon"));
        assert!(content.contains("ExecStart=%h/.cargo/bin/vigil-daemon"));
    } else {
...
async fn test_uninstall_cleanups() {
    ...
    let config_dir = vigil_lib::paths::config_dir();
    let log_path = vigil_lib::paths::log_path();
