...
fn test_logs_basic() {
    ...
    let data_dir = temp.path().join("vigil");
    fs::create_dir_all(&data_dir).unwrap();
    let log_path = data_dir.join("vigil.log");
...
fn test_logs_rotation_selection() {
    ...
    let rotated_log = data_dir.join("vigil.log.2026-01-29");
    ...
    let active_log = data_dir.join("vigil.log");
