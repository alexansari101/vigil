...
"#,
        ...
        let workspace_root = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            .unwrap()
            .to_path_buf();
        let daemon_path = workspace_root.join("target/debug/vigil-daemon");

        // Ensure daemon path exists, if not try just 'vigil-daemon' (might be in PATH or different target)
        let daemon_path = if daemon_path.exists() {
            daemon_path
        } else {
            // Fallback for different build profiles or environments
            std::path::PathBuf::from("vigil-daemon")
        };
    ...
    fn run_cli(&self, args: &[&str]) -> Result<(bool, String, String)> {
        let workspace_root = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            .unwrap()
            .to_path_buf();
        let cli_path = workspace_root.join("target/debug/vigil");
        ...
        let output = Command::new(cli_path)
            ...
            .env(
                "VIGIL_CONFIG",
                self.temp_dir.path().join("config/vigil/config.toml"),
            )
            .output()?;
