...
use vigil_lib::config::{BackupSet, Config, GlobalConfig};
use vigil_lib::paths;
use vigil_lib::types::JobState;
...
/// ```bash
/// cargo test -p vigil-daemon --test integration_test -- --ignored --test-threads=1
/// ```
#[tokio::test]
#[ignore]
#[serial]
...
async fn test_auto_prune_after_backup() -> Result<()> {
    use vigil_lib::config::RetentionPolicy;
